...
struct RvalueRefTest {
  template<class T>
  static int f_rref_template(T&& val) {
    return val.x;
  }
  template<class T>
  static void f_rref_template_mutates(T&& val) {
    val.x = 17;
  }
  template<class T>
  static typename std::remove_reference<T>::type f_rref_template_copies(T&& val) {
    // Here T&& is a 'univeral/forwarding reference', so won't 
    // necessarily bind to an rvalue, but can also bind lvalues.
    // Either
    //   a) T copy = val; // with T=Something& this actually won't create a copy!
    // or
    //   b) std::remove_reference<T>::type copy = val;
    static_assert(std::is_reference<T>::value, ""); // this may or may not be satisfied,
                                                    // depends on caller
    typename std::remove_reference<T>::type copy = val;
    static_assert(!std::is_reference<decltype(copy)>::value, ""); // guaranteed by remove_reference()
    copy.x = 18;
    return copy;
  }
  template<class T>
  static T f_rref_template_moves(T&& val) {
...
void play_with_cpp11() {
  ...
  // This corresponds to 'for x in iterable' in Python and JS.
  {
    vector<int> vec = {5,6};
    {
      int total = 0;
      ...
      assert(total == 11);
      assert(std::accumulate(vec.begin(), vec.end(), 0, std::plus<int>()) == 11);
    }
    {
      int total = 0;
      for (int& x : vec) {
      ...
      assert(total == 11);
      assert(std::accumulate(vec.begin(), vec.end(), 0, std::plus<int>()) == 21);
    }
    // replace 'int' with 'auto' for the same behavior, so
    // for (auto& x : vec) ...
  ...
  // Good intro http://www.aristeia.com/TalkNotes/ACCU2011_MoveSemantics.pdf and
  // at http://thbecker.net/articles/rvalue_references/section_01.html
  // Details http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2010/n3053.html
  //
  // Facts to remember:
  // * move is simply an optimization of copy, one which allows gutting the
  //   rhs object (leaving it in a destructible & assignable but otherwise
  //   undefined state)
  // * there are very few classes which are moveable but not copyable, one 
  //   example is std::unique_ptr<> (copying it would make it non-unique), which
  //   is one of deprecated auto_ptr's spiritual successors.
  // * pre-C++11 we had copy assignment op & copy ctor, since C++11 we have
  //   additional       move assignemnt op & move ctor.
  // * the compiler will autogenerate move assignment op & move ctor if
  //   all base classes and members are movable. Which is the same rule as for
  //   autogenerated copy assignment op & copy ctor.
  // * remember std::move to cast an lvalue ref to an rvalue (or xvalue) ref,
  //   declaring the std::move arg 'guttable'
  // * http://google-styleguide.googlecode.com/svn/trunk/cppguide.html suggest
  //   to not bother with using std::forward in ctors or setters for perfect 
  //   forwarding (see http://www.aristeia.com/TalkNotes/ACCU2011_MoveSemantics.pdf)
  //   I wonder why not? Code bloat from O(2^N) ctors? Gain not worth it?
  //   Because it forces you to pull the impl of ctors & settings into .h file
  //   (since the setters and ctors need to be templatized to be able to use
  //   std::forward).
  // * worry about exception safety & guarantees during moves
  //   e.g. see http://en.cppreference.com/w/cpp/language/noexcept_spec (with noexecpt
  //   replacing C++98's deprecated throw() spec). So declare your move ctors
  //   and move assignment ops as noexecpt if possible, see
  //   http://www.codingstandard.com/rule/12-5-4-declare-noexcept-the-move-constructor-and-move-assignment-operator/
  //     "For example, containers such as std::vector will move their elements 
  //     if the elements' move constructor is noexcept, and copy otherwise"
  ...
  }
  {
    ...
    // Thir will get a compiler-generated move assignment & ctors if its members
    // allow for these.
    struct S {
      int a;
      S(const S&) {}
  ...
  {
    // I thought having a user-defined dtor will prevent the compiler from
    // adding a move ctor and move assignment op, but cannot confirm this:
    struct S {
      ~S() {}
      int a;
      vector<int> v;
  ...
  {
    // Actually here's an example where adding a user-defined ctor will prevent
    // the compiler from adding a move ctor & assignment op, but it requires
    // having an std::unique_ptr member, which is movable but not copyable:
    struct S {
      int a;
      std::unique_ptr<int> p;
      //~S() {} // if you add this line then S is no longer movable
    };
    static_assert(!std::is_copy_constructible<S>::value, "");
    static_assert( std::is_move_constructible<S>::value, "");
    static_assert(!std::is_copy_assignable<S>::value, "");
    ...
    S s1;
    S s2;
    //s2 = s1; // cannot be assigned because its copy assignment operator is implicitly deleted
  }
  {
  ...
  {
    // 'forwarding references' aka Scott Meyers 'Universal References'
    ...
    auto f_cref = [](const A& a) {
      int x = a.x;
    };
    auto f_rref = [](A&& a) {
      a.x = 8; // kinda pointless, since a should have bound to a temporary,
               // so client likely cannot observe the value unless he used
               // std::move
    };
    auto f_ret = []() -> A {
      return A(2);
    ...
    f_lref(a);
    assert(a.x == 7);
    //f_lref(ca); // err: candidate function not viable: 1st argument ('const A') would lose const qualifier
    f_cref(a);
    f_cref(ca);
    //f_rref(a); // err: candidate function not viable: no known conversion from 'A' to 'A &&' for 1st argument
    f_cref(f_ret()); // good: a temporary aka rvalue can bind to a const ref since C++98
    //f_lref(f_ret()); // good: a temporary nvr could bind to a const ref since C++98
                     // err: candidate function not viable: expects an l-value for 1st argument
    f_rref(f_ret()); // good: C++11 will bind temporary to rvalue ref, gutting its value
    
    assert(a.x == 7);
    f_rref(std::move(a));
    assert(a.x == 8); // std::move allowed gutting 'a'

    //f_rref(std::move(ca)); // err: argument would lose const qualifier, good

    // So here it gets weird imo: see how f_rref above could not bind its arg to
    // an lvalue? Move exactly the same func into a template and suddenly it
    // can bind to anything! This makes me kinda dislike rvalue refs atm :(
    // Anyway, that's why rvalue refs in templates are named forwarding/universal
    // references.
    RvalueRefTest::f_rref_template(a);
    RvalueRefTest::f_rref_template(ca);
    RvalueRefTest::f_rref_template(f_ret());

    // you still cannot violate constness with this forwarding reference, since
    // when compiling the method's body you'll get errors:
    RvalueRefTest::f_rref_template_mutates(a);
    assert(a.x == 17);
    //RvalueRefTest::f_rref_template_mutates(ca); // err: read-only variable is not assignable
    
    RvalueRefTest::f_rref_template_copies(a);
    assert(a.x == 17); // without std::remove_reference this was 18, so what looked like
                       // a copy in the template impl was not, because T==A&

    a.x = 17;
    //RvalueRefTest::f_rref_template_copies(a); // not viable: no known conversion from 'A' to 'A &&'
    //assert(a.x == 17);
    
    // So for now I'm tempted to avoid using 'universal=forwarding references'.
  }
  // So when & how do we actually bother to impl move assignment op & ctor?
  // Only if the object holds on to resources that are not cheaply copyable
  // and are not moveable themselves alrdy? Note that std::string & std::vector
  // alrdy are movable, so if your class contains these don't bother adding
  // move ctors. Details https://akrzemi1.wordpress.com/2011/08/30/move-constructor-qa/
  // So I guess you'd implement move ctors & assignment ops only if you implemented
  // your own non-standard container (e.g. an alternative for std::vector or 
  // std::map). Which is very rare for most C++ devs.
  {
    // An example is here: http://msdn.microsoft.com/en-us/library/dd293665.aspx
    // So don't forget the 'if (this != &rhs)' test (same as for copy assignment op)
    // And don't forget using std::move(rhs), even if rhs alrdy is of type T&&.
  }
...
//#endif
  ...
  // http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2007/n2249.html
  {
    ...
    // C++11:
    const char* s8 = u8"foo"; // UTF8 encoded byte seq
    const char16_t* s16 = u"foo";
    const char32_t* s32 = U"foo";
    string u8str(s8); // string.length() will return len in bytes, not unicode chars!
